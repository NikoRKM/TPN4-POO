		...
		HashMap<String, Empleado> empleado= new HashMap<String, Empleado>(); 
	        ...
	        do {
	            ...
	            if (option == 1) {
	                ...
	                //Aquí se asigna a la variable key la clave del empleado (concatenacion entre el dni y la categoria)
	                key=dni+""+categoria;
	                variante.setKey(key);

	                if ( empleado.containsKey(dni)) {
	        	        System.out.println("No se puede introducir el empleado. El dni esta repetido.");
	        	    } else {
	        	    	empleado.put(key, variante);          
	        	    }
	                ...
	                while(listado.hasNext()){
	                    lista = listado.next();
	                    System.out.println(lista + " - " + empleado.get(listado));
	                } 
